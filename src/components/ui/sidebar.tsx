...
function Sidebar({
  ...
  const slideAnimationClasses =
    collapsible === "offcanvas"
      ? side === "left"
        ? "group-data-[state=collapsed]:-translate-x-full group-data-[state=expanded]:translate-x-0"
        : "group-data-[state=collapsed]:translate-x-full group-data-[state=expanded]:translate-x-0"
      : ""
    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        <SheetContent
          ...
          data-slot="sidebar"
          data-mobile="true"
          className="bg-white text-sidebar-foreground p-0 w-70 border-r-0"
          side={side}
        ...
        >
          ...
          <div className="flex h-full w-full flex-col pt-7.5">{children}</div>
        </SheetContent>
      </Sheet>
    )
  }

  // collapsible="none"の場合はデスクトップで常に表示
  if (collapsible === "none") {
    return (
      <div
        data-slot="sidebar"
        className={cn(
          "bg-sidebar text-sidebar-foreground flex h-full w-(--sidebar-width) flex-col hidden lg:flex",
          className
        )}
        {...props}
      >
